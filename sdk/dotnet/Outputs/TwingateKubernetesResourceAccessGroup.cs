// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Twingate.Twingate.Outputs
{

    [OutputType]
    public sealed class TwingateKubernetesResourceAccessGroup
    {
        /// <summary>
        /// Restrict access according to JIT access policy
        /// </summary>
        public readonly ImmutableArray<Outputs.TwingateKubernetesResourceAccessGroupAccessPolicy> AccessPolicies;
        /// <summary>
        /// Group ID that will have permission to access the Resource.
        /// </summary>
        public readonly string? GroupId;
        /// <summary>
        /// The ID of a `twingate.getTwingateSecurityPolicy` to use as the access policy for the group IDs in the access block. Default is 'Null' which points to `Default Policy` on Admin console.
        /// </summary>
        public readonly string? SecurityPolicyId;

        [OutputConstructor]
        private TwingateKubernetesResourceAccessGroup(
            ImmutableArray<Outputs.TwingateKubernetesResourceAccessGroupAccessPolicy> accessPolicies,

            string? groupId,

            string? securityPolicyId)
        {
            AccessPolicies = accessPolicies;
            GroupId = groupId;
            SecurityPolicyId = securityPolicyId;
        }
    }
}
